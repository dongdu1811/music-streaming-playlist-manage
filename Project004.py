...
class MusicPlayer:
    ...
    def find_by_title(self, title):
        ...
        while leftsearch is not None:
            title2 = leftsearch.song.title.lower().replace(" ", "")
            matches = 0
            if title2 == title:
                print(f"Found {leftsearch.song}")
                print(f"Song ID is [{leftsearch.song.song_id}]")
                return leftsearch
            elif title in title2:
                print(f"Found {leftsearch.song}")
                print(f"Song ID is [{leftsearch.song.song_id}]")
                return leftsearch
            for letter in set(title):
                if letter in title2:
                    matches += 1
            if matches / len(title2) >= 0.6:            
                print(f"Found {leftsearch.song}")
                print(f"Song ID is [{leftsearch.song.song_id}]")
                return leftsearch
            leftsearch = leftsearch.next
        print("Song Doesn't Exist in Library")
        return
    ...
    def menu(self):
        ...
        while True:
            ...
            if choice == '1':
                ...
                self.find_by_title(input(f"Please Enter Song Title:"))
            elif choice == '5':
                self.playlist.display()
            elif choice == '6':
                self.play_next()
